...
import (
	...
	"github.com/zbigniew73/litecp/internal/agent"
	"github.com/zbigniew73/litecp/internal/crypto"
	"github.com/zbigniew73/litecp/internal/database"
)
...
func automaticWWWRedirectDomain(domain string) string {
	d := strings.ToLower(strings.TrimSpace(domain))
	if d == "" || strings.HasPrefix(d, "www.") || isLocalDomain(d) {
		return ""
	}
	return "www." + d
}
...
func (s *SiteService) Create(ctx context.Context, req *CreateSiteRequest) (*Site, error) {
	...
	if exists, err := s.db.DomainExists(ctx, domain); err != nil {
		return nil, fmt.Errorf("failed to check domain availability: %w", err)
	} else if exists {
		return nil, fmt.Errorf("domain '%s' is already in use", domain)
	}

	wwwDomain := automaticWWWRedirectDomain(domain)
	if wwwDomain != "" {
		if exists, err := s.db.DomainExists(ctx, wwwDomain); err != nil {
			return nil, fmt.Errorf("failed to check www domain availability: %w", err)
		} else if exists {
			return nil, fmt.Errorf("www alias '%s' is already in use", wwwDomain)
		}
	}
	...
	if siteType == "wordpress" {
		...
		// Save database record to LiteCP database so it appears in the UI
	...
	// Add primary domain and automatic www redirect alias to site_domains table.
	...
	if wwwDomain != "" {
		wwwRedirectDomain := &database.SiteDomain{
			SiteID:            id,
			Domain:            wwwDomain,
			IsPrimary:         false,
			RedirectToPrimary: true,
		}
		if err := s.db.AddSiteDomain(ctx, wwwRedirectDomain); err != nil {
			fmt.Printf("warning: failed to add automatic www redirect domain: %v\n", err)
		} else {
			createdDomains = append(createdDomains, SiteDomain{
				ID:                wwwRedirectDomain.ID,
				SiteID:            id,
				Domain:            wwwDomain,
				IsPrimary:         false,
				RedirectToPrimary: true,
				CreatedAt:         wwwRedirectDomain.CreatedAt,
			})
		}
	}
