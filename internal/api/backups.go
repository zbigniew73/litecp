...
func loadSnapshotManifest(ctx context.Context, run func(context.Context, string) ([]byte, error), resticCmd, snapshotID string) (*backupManifest, error) {
	manifestPath, err := resolveSnapshotPathBySuffixUnique(ctx, run, resticCmd, snapshotID, filepath.Join(".litecp", "backups", "manifest.json"), false)
	if err != nil {
		return nil, err
	}
...
func (s *BackupService) runBackup(username string, cfg *database.BackupConfig) (string, string, error) {
	...
	backupRoot := filepath.Join(homeDir, ".litecp", "backups")
	...
	backupScript := fmt.Sprintf("%s backup --host litecp --json %s %s %s", resticCmd, strings.Join(tagArgs, " "), strings.Join(excludeArgs, " "), strings.Join(quotedPaths, " "))
	...
	forgetArgs := []string{resticCmd + " forget --group-by '' --tag " + shellQuote("litecp")}
	if cfg.KeepLast > 0 {
	...
	if snapshotID == "" {
		snapshotsJSON, snapErr := s.runWithResticLockRecovery(ctx, username, env, cfg,
			fmt.Sprintf("%s snapshots --json --tag %s --latest 1", resticCmd, shellQuote("litecp")))
		if snapErr != nil {
...
func (s *BackupService) ListSnapshots(ctx context.Context, username string, limit int) ([]BackupSnapshot, error) {
	...
	output, cmdErr := s.runWithResticLockRecovery(ctx, username, env, cfg, fmt.Sprintf(
		...
		resticCmd, shellQuote("litecp"), limit))
	if cmdErr == nil {
...
func (s *BackupService) CreateSnapshotZip(ctx context.Context, username, snapshotID string) (string, string, error) {
	...
	downloadRoot := filepath.Join(homeDir, ".litecp", "backups", "downloads")
	...
	manifestPathInSnapshot, manifestPathErr := resolveSnapshotPathBySuffixUnique(
		...
		resticCmd,
		snapshotID,
		filepath.Join(".litecp", "backups", "manifest.json"),
		false,
	)
...
func (s *BackupService) RestoreSite(ctx context.Context, username string, req *RestoreSiteRequest) (string, error) {
	...
	go func() {
		s.sem <- struct{}{}
		defer func() { <-s.sem }()
		restoreTarget := filepath.Join(homeDir, ".litecp", "backups", "restore", jobID)
		env, envErr := s.resticEnv(cfg, homeDir, pass)
		if envErr != nil {
